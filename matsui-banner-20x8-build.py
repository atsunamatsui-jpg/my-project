"""Builds the 20ft x 8ft tradeshow banner.
...
TRIM_W, TRIM_H = 6000, 2400        # 240 x 96 in = 20 x 8 ft
PX_PER_IN = 25
BLEED_IN, SAFE_IN = 2, 4
BLEED = BLEED_IN * PX_PER_IN       # 50 px
SAFE  = SAFE_IN * PX_PER_IN        # 100 px
W, H = TRIM_W + BLEED*2, TRIM_H + BLEED*2   # 6100 x 2500 = 244 x 100 in
...
BOT_PAD = 40                       # content no longer runs to the hem, so the
...
# ---- consumables lockup -----------------------------------------------------
# Matsui sits beside "DTF CONSUMABLES" rather than above it, with MATSUI matched
# to the headline's cap height and sitting on the same baseline. "MATSUI" is one
# line inside logo_full.png, so the logo's overall height is solved from it:
...
# The font-size is then chosen so logo + gap + title spans the column, which is
# what removes the empty space either side.
CONS_COL = 2288
CONS_LOCK_GAP = 64
CONS_TITLE_W100 = 965.5              # "DTF CONSUMABLES" at font-size 100
LOGO_AR = 860 / 441
MATSUI_CAP_FRAC, MATSUI_TOP_FRAC = 0.2018, 0.6508
CAP_RATIO, INK_TOP_RATIO = 0.6908, 0.1513

CONS_FS = round((CONS_COL - 138 - CONS_LOCK_GAP)
                / (CONS_TITLE_W100 / 100 + CAP_RATIO / MATSUI_CAP_FRAC * LOGO_AR))
CONS_LOGO_H = round(CONS_FS * CAP_RATIO / MATSUI_CAP_FRAC)
CONS_OFFSET = round(CONS_LOGO_H * MATSUI_TOP_FRAC - CONS_FS * INK_TOP_RATIO)
...
.col-mm4  {{ width:1659px; padding:{SAFE}px 0 0; }}
.col-mm2  {{ width:1453px; padding:{SAFE}px 0 0; }}
.col-cons {{ width:2288px; padding:{SAFE}px 0 0; }}
...
.lockup-mm4 {{ width:100%; max-width:1100px; }}
.lockup-mm2 {{ width:100%; max-width:957px; margin-top:10px; }}

.hl {{ text-align:center; margin-top:26px; }}
.hl .main {{
  display:block; font-family:'Display',sans-serif; font-weight:700;
  font-size:88px; letter-spacing:-1.5px; line-height:1;
...
.rainbow {{ text-align:center; margin-top:16px; }}
.rainbow span {{
  display:block; font-family:'Display',sans-serif; font-weight:700;
  font-size:44px; letter-spacing:0.5px; line-height:1.28;
}}
...
.feats {{ display:flex; flex-direction:row; justify-content:center;
  align-items:stretch; gap:14px; width:100%; margin-top:22px; }}
.feat {{
...
.feat-solo {{ flex:0 0 auto; padding:14px 32px; }}
.col-mm2 .feats {{ margin-top:20px; }}
...
.col-mm2 .feat .ic {{ position:absolute; left:22px; top:50%; transform:translateY(-50%); }}
.feat .ic {{ width:54px; height:54px; flex:none; display:block; }}
.feat .ic svg {{ width:100%; height:100%; display:block; }}
.ft {{ display:flex; flex-direction:column; line-height:1.24; text-align:center; }}
.ft b {{ font-family:'Body',sans-serif; font-weight:700; font-size:34px; letter-spacing:1.5px; color:#fff; }}
...
.cons-top {{
  ...
  justify-content:center; gap:{CONS_LOCK_GAP}px;
}}
.cons-top img.logo {{ height:{CONS_LOGO_H}px; display:block;
  filter:drop-shadow(0 12px 30px rgba(0,0,0,0.55)); }}
.cons-title {{
  font-family:'Display',sans-serif; font-weight:700; font-size:{CONS_FS}px;
  letter-spacing:-0.5px; line-height:1; white-space:nowrap;
  margin-top:{CONS_OFFSET}px; color:#fff;
  text-shadow:0 8px 26px rgba(0,0,0,0.45);
}}
.cons-title span {{ color:#FFE28A; }}
.cons-rule {{
  width:100%; height:2px; margin:20px auto 24px;
...
.clab {{
  margin-top:14px; font-family:'Body',sans-serif; font-weight:700;
  font-size:38px; letter-spacing:2px; color:#F2F7FA; text-align:center; line-height:1.25;
}}
'''
...
GUIDE_HTML = f'''
  <div class="guides">
    ...
    <div class="g-tag g-tag-trim">TRIM 240 x 96 in</div>
    <div class="g-tag g-tag-bleed">BLEED {BLEED_IN}in — 244 x 100 in</div>
...
<head>
<meta charset="UTF-8"/>
<title>Matsui DTF Tradeshow Banner 20x8</title>
