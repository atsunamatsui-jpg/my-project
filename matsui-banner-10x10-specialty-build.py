"""Builds the 10ft x 10ft (1:1) Specialty Inks tradeshow banner.
...
# Trim (finished) size is 120x120in mapped to 3000x3000 css px -> 25 px/in,
# the same scale as the 20x10 banner so type sizes carry across directly.
TRIM_W, TRIM_H = 3000, 3000        # 120 x 120 in
PX_PER_IN = 25
BLEED_IN, SAFE_IN = 2, 4
BLEED = BLEED_IN * PX_PER_IN       # 50 px
SAFE = SAFE_IN * PX_PER_IN         # 100 px
W, H = TRIM_W + BLEED*2, TRIM_H + BLEED*2      # 3100 x 3100 = 124 x 124 in
...
HEAD_FS = 140                                   # headline font-size
...
.lock {{ display:flex; flex-direction:row; align-items:flex-start; gap:56px; }}
...
.head .rule {{
  width:100%; height:4px; margin-top:22px;
  background:linear-gradient(90deg,rgba(255,226,138,0) 0%,#FFE28A 50%,rgba(255,226,138,0) 100%);
}}

/* ---- product grid ---- */
.grid {{
  flex:1; min-height:0; width:100%; margin-top:28px;
  display:grid; grid-template-columns:repeat(2,1fr); grid-auto-rows:1fr; gap:30px 36px;
}}
.card {{
...
.info {{ flex:1; min-width:0; padding:16px 28px; display:flex; flex-direction:column; justify-content:center; }}
.info h3 {{
  font-family:'Display',sans-serif; font-weight:700; font-size:56px;
  line-height:1.08; letter-spacing:-0.5px; color:#fff;
}}
.info ul {{ list-style:none; margin-top:16px; }}
.info li {{
  font-family:'Body',sans-serif; font-weight:400; font-size:45px; line-height:1.34;
  color:rgba(255,255,255,0.90); padding-left:34px; position:relative;
}}
.info li::before {{
  content:""; position:absolute; left:4px; top:0.55em;
  width:13px; height:13px; border-radius:50%; background:#FFE28A;
}}
.spec {{
  margin-top:18px; padding-top:16px; border-top:1px solid rgba(255,255,255,0.24);
  display:flex; flex-direction:column;
}}
.spec .base {{
  font-family:'Body',sans-serif; font-weight:700; font-size:40px; letter-spacing:0.5px;
  color:#FFE28A; line-height:1.25;
}}
.spec .nums {{
  display:flex; flex-wrap:wrap; align-items:baseline; column-gap:14px;
  font-family:'Body',sans-serif; font-weight:400; font-size:36px; letter-spacing:0.5px;
  color:rgba(255,255,255,0.82); line-height:1.3;
}}
.spec .nums b {{ font-weight:400; white-space:nowrap; }}
.spec .nums i {{ font-style:normal; opacity:0.5; }}
'''
...
GUIDE_HTML = f'''
  <div class="guides">
    ...
    <div class="g-tag g-tag-trim">TRIM 120 x 120 in</div>
    <div class="g-tag g-tag-bleed">BLEED {BLEED_IN}in — 124 x 124 in</div>
...
<head>
<meta charset="UTF-8"/>
<title>Matsui Specialty Inks Tradeshow Banner 10x10</title>
